...
- [ ] **実スキャンの取り込み仕上げ(継続)**: 一次デモ(scan_demo.py: 軸合わせ+符号なし距離での配置・可視化)まで実装済み。残り — 開口部への蓋付けによる水密化(符号付き距離)、進行方向の軸合わせ自動化、START/GOAL定義、RRTでの経路探索、メジャー実測との精度突き合わせ
...
- [x] 実スキャン階段の一次デモ(scan_demo.py: cm正規化・Z-up軸合わせ・符号なし距離での配置と可視化。実データで家具配置とクリアランスが出た) (2026-09-19, feature/scan-demo)
